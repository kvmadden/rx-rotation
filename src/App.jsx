...
import { Btn, Inp, Sel, TSel, Tog, NW, Card, Collapsible, SegBtn } from "./components/ui.jsx";
...
<style>{"@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500;600;700&display=swap'); @keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}"}</style>
{/* HEADER BAR */}
...
{[{ l: "Coverage", w: "30%", c: Co.ac, d: "Every open hour has a pharmacist on duty" }, { l: "Demand Fit", w: "20%", c: Co.gn, d: "Scheduled hours match demand \u2014 no under or over-scheduling" }, { l: "Fairness", w: "15%", c: Co.tl, d: "Weekends, opens, and closes distributed equitably across team" }, { l: "Peak Strength", w: "15%", c: Co.am, d: "Strongest pharmacist on duty during highest-traffic windows" }, { l: "Sustainability", w: "10%", c: Co.tl, d: "Each pharmacist\u2019s scheduled hours stay close to their target" }, { l: "Preferences", w: "10%", c: Co.pu, d: "Individual pharmacist preferences honored where possible" }].map(function (f) {
...
<SegBtn active={!store._hoursChanged} onClick={function () { setStore(function (s) { return { ...s, _hoursChanged: false, allocatedHoursPerWeek: s._originalBudget || s.allocatedHoursPerWeek }; }); }}>Same for new template</SegBtn>
<SegBtn active={store._hoursChanged} color={Co.am} borderLeft onClick={function () { setStore(function (s) { return { ...s, _hoursChanged: true, _originalBudget: s._originalBudget || s.allocatedHoursPerWeek }; }); }}>Changed</SegBtn>
</div>
...
<div className="seg-control" style={{ border: "1px solid " + Co.bdr }}>
<SegBtn active={!store._rotChanged} onClick={function () { setStore(function (s) { return { ...s, _rotChanged: false, rotationWeeks: s._originalRot || s.rotationWeeks }; }); }}>Same for new template</SegBtn>
<SegBtn active={store._rotChanged} color={Co.am} borderLeft onClick={function () { setStore(function (s) { return { ...s, _rotChanged: true, _originalRot: s._originalRot || s.rotationWeeks }; }); }}>Changed</SegBtn>
</div>
...
{teamWarning && !teamWarning.includes("to continue") ? <div role="alert" aria-live="assertive" style={{ padding: "12px 14px", background: Co.amS, borderRadius: 10, marginBottom: 12, border: "1px solid " + Co.am + "40", transition: "transform 0.15s, box-shadow 0.15s", transform: flash.team ? "scale(1.03)" : "none", boxShadow: flash.team ? "0 0 0 2px " + Co.am + "60" : "none" }}><div style={{ fontSize: 12, color: Co.am, fontWeight: 600, marginBottom: 8 }}>{teamWarning}</div><div style={{ display: "flex", gap: 8 }}><Btn style={{ padding: "5px 12px", fontSize: 11, flex: 1 }} onClick={function () { setTeamWarning(null); }}>Go back</Btn><Btn variant="primary" style={{ padding: "5px 12px", fontSize: 11, flex: 1 }} onClick={function () { setTeamWarning(null); pushHistory(); if (mode === "improve") { initCurSched(); } setStore(function (s) { return { ...s, _originalBudget: s.allocatedHoursPerWeek }; }); setStep(4); window.scrollTo({ top: 0, behavior: "smooth" }); }}>Continue anyway</Btn></div></div> : null}
...
if (pharms.length === 0) return <div>{teamWarning && teamWarning.includes("to continue") ? <div role="alert" aria-live="assertive" style={{ padding: "10px 14px", background: Co.rdS, borderRadius: 10, marginBottom: 8, border: "1px solid " + Co.rd + "40", transition: "transform 0.15s, box-shadow 0.15s", transform: flash.team ? "scale(1.03)" : "none", boxShadow: flash.team ? "0 0 0 2px " + Co.rd + "60" : "none" }}><div style={{ fontSize: 12, color: Co.rd, fontWeight: 600 }}>{teamWarning}</div></div> : null}<Btn variant="primary" onClick={addPharm} style={{ width: "100%", marginTop: 6 }}>+ Add Pharmacy Manager</Btn></div>;
if (!teamComplete) return <div>{teamWarning && teamWarning.includes("to continue") ? <div role="alert" aria-live="assertive" style={{ padding: "10px 14px", background: Co.rdS, borderRadius: 10, marginBottom: 8, border: "1px solid " + Co.rd + "40", transition: "transform 0.15s, box-shadow 0.15s", transform: flash.team ? "scale(1.03)" : "none", boxShadow: flash.team ? "0 0 0 2px " + Co.rd + "60" : "none" }}><div style={{ fontSize: 12, color: Co.rd, fontWeight: 600 }}>{teamWarning}</div></div> : null}<Btn variant="primary" onClick={addPharm} style={{ width: "100%", marginTop: 6 }}>+ Add Next Pharmacist</Btn></div>;
...
<div role="button" tabIndex={0} aria-pressed={!hasCustomRange} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { var b = +p.targetHours || 40; return { ...p, _showRange: false, minHours: b - 8, maxHours: b + 8 }; }); } }} onClick={function () { setEditP(function (p) { var b = +p.targetHours || 40; return { ...p, _showRange: false, minHours: b - 8, maxHours: b + 8 }; }); }} style={{ flex: 1, padding: "10px 8px", fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "center", lineHeight: 1.4, borderRadius: 8, background: !hasCustomRange ? Co.ac : Co.card, color: !hasCustomRange ? "#fff" : Co.txMu, border: "1px solid " + (!hasCustomRange ? Co.ac : Co.bdr) }}>Maintain current<br />weekly base hours</div>
<div role="button" tabIndex={0} aria-pressed={!!hasCustomRange} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { return { ...p, _showRange: true }; }); } }} onClick={function () { setEditP(function (p) { return { ...p, _showRange: true }; }); }} style={{ flex: 1, padding: "10px 8px", fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "center", lineHeight: 1.4, borderRadius: 8, background: hasCustomRange ? Co.am : Co.card, color: hasCustomRange ? "#fff" : Co.txMu, border: "1px solid " + (hasCustomRange ? Co.am : Co.bdr) }}>Flexible within<br />acceptable range</div>
</div>
...
<SegBtn active={!hasRange} onClick={function () { setEditP(function (p) { var b = +p.targetHours || 40; return { ...p, _showRange: false, minHours: b - 8, maxHours: b + 8 }; }); }}>Exact hours</SegBtn>
<SegBtn active={hasRange} color={Co.am} borderLeft onClick={function () { setEditP(function (p) { return { ...p, _showRange: true }; }); }}>Acceptable range</SegBtn>
</div>
...
{editP.role === "dsp" ? <Card style={{ padding: 14 }}><div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>Days available</div><div style={{ display: "flex", gap: 4 }}>{DAYS.map(function (d) { var isOn = !(editP.prefs.fixedDaysOff || []).includes(d); return <div key={d} role="button" tabIndex={0} aria-pressed={isOn} aria-label={d + (isOn ? " available" : " unavailable")} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { var fix = [].concat(p.prefs.fixedDaysOff); if (isOn) { fix.push(d); } else { fix = fix.filter(function (x) { return x !== d; }); } return { ...p, prefs: { ...p.prefs, fixedDaysOff: fix } }; }); } }} onClick={function () { setEditP(function (p) { var fix = [].concat(p.prefs.fixedDaysOff); if (isOn) { fix.push(d); } else { fix = fix.filter(function (x) { return x !== d; }); } return { ...p, prefs: { ...p.prefs, fixedDaysOff: fix } }; }); }} style={{ padding: "8px 0", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer", textAlign: "center", flex: 1, background: isOn ? Co.gnS : Co.bg, color: isOn ? Co.gn : Co.txD, border: "1px solid " + (isOn ? Co.gn + "30" : "transparent") }}>{d}</div>; })}</div></Card> : (
<div>
...
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><Tog value={editP.prefs.noClopening} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, noClopening: v } }; }); }} /><span style={{ fontSize: 13, flex: 1 }}>11+ hours between shifts</span>{editP.prefs.noClopening ? <NW value={(editP.prefs.needs || {}).noClopening} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, needs: { ...p.prefs.needs, noClopening: v } } }; }); }} /> : null}</div>
{[{ k: "preferEarly", l: store.is24hr ? "Prefers early shift" : "Prefers opening" }, { k: "preferLate", l: store.is24hr ? "Prefers late shift" : "Prefers closing" }].map(function (pr) { var isOn = editP.prefs[pr.k]; return <div key={pr.k} className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><Tog value={isOn} onChange={function (v) { setEditP(function (p) { var newPrefs = { ...p.prefs, [pr.k]: v }; if (v && pr.k === "preferEarly") { newPrefs.preferLate = false; } if (v && pr.k === "preferLate") { newPrefs.preferEarly = false; } return { ...p, prefs: newPrefs }; }); }} /><span style={{ fontSize: 13, flex: 1 }}>{pr.l}</span></div>; })}
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><Tog value={editP.prefs.noBackToBackLong} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, noBackToBackLong: v } }; }); }} /><span style={{ fontSize: 12, flex: 1 }}>No back-to-back long shifts</span></div>
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><Tog value={editP.prefs.threeDayWeekend} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, threeDayWeekend: v } }; }); }} /><span style={{ fontSize: 12, flex: 1 }}>Prefers 3-day weekend block</span></div>
{/* DAYS OFF */}
...
<div style={{ display: "flex", gap: 4, marginBottom: 10 }}>{DAYS.map(function (d) { var isFixed = editP.prefs.fixedDaysOff.includes(d); var isPref = (editP.prefs.preferredDaysOff || []).includes(d); var chipColor = isFixed ? Co.rd : isPref ? Co.am : null; return <div key={d} role="button" tabIndex={0} aria-label={d + (isFixed ? " needs off" : isPref ? " prefers off" : " flexible")} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { var fix = [].concat(p.prefs.fixedDaysOff); var pref = [].concat(p.prefs.preferredDaysOff || []); if (!isPref && !isFixed) { pref.push(d); } else if (isPref && !isFixed) { pref = pref.filter(function (x) { return x !== d; }); fix.push(d); } else { fix = fix.filter(function (x) { return x !== d; }); } return { ...p, prefs: { ...p.prefs, fixedDaysOff: fix, preferredDaysOff: pref } }; }); } }} onClick={function () { setEditP(function (p) { var fix = [].concat(p.prefs.fixedDaysOff); var pref = [].concat(p.prefs.preferredDaysOff || []); if (!isPref && !isFixed) { pref.push(d); } else if (isPref && !isFixed) { pref = pref.filter(function (x) { return x !== d; }); fix.push(d); } else { fix = fix.filter(function (x) { return x !== d; }); } return { ...p, prefs: { ...p.prefs, fixedDaysOff: fix, preferredDaysOff: pref } }; }); }} style={{ padding: "5px 0", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "center", flex: 1, background: chipColor ? chipColor + "15" : Co.bg, color: chipColor || Co.txMu, border: "1px solid " + (chipColor ? chipColor + "30" : "transparent") }}>{d}</div>; })}</div>
...
{Object.keys(editP.prefs.dayOverrides).map(function (d) { var ovr = editP.prefs.dayOverrides[d]; return <div key={d} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 0", borderBottom: "1px solid " + Co.bdrL }}><div style={{ width: 32, fontSize: 12, fontWeight: 600 }}>{d}</div><span style={{ fontSize: 11, color: Co.tl, fontWeight: 600 }}>By</span><TSel value={ovr.latestEnd} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, dayOverrides: { ...p.prefs.dayOverrides, [d]: { ...p.prefs.dayOverrides[d], latestEnd: v } } } }; }); }} /><Inp value={ovr.note} onChange={function (e) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, dayOverrides: { ...p.prefs.dayOverrides, [d]: { ...p.prefs.dayOverrides[d], note: e.target.value } } } }; }); }} placeholder="Reason" style={{ fontSize: 11, flex: 1, minWidth: 50, padding: "3px 6px" }} maxLength={30} /><span role="button" tabIndex={0} aria-label={"Remove " + d + " override"} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { var o = { ...p.prefs.dayOverrides }; delete o[d]; return { ...p, prefs: { ...p.prefs, dayOverrides: o } }; }); } }} onClick={function () { setEditP(function (p) { var o = { ...p.prefs.dayOverrides }; delete o[d]; return { ...p, prefs: { ...p.prefs, dayOverrides: o } }; }); }} style={{ fontSize: 12, color: Co.txD, cursor: "pointer" }}>{"\u2715"}</span></div>; })}
{(function () { var avail = DAYS.filter(function (d) { return !Object.keys(editP.prefs.dayOverrides).includes(d); }); if (avail.length === 0) return null; return <div style={{ marginTop: 6 }}><span role="button" tabIndex={0} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setEditP(function (p) { var d = DAYS.filter(function (x) { return !Object.keys(p.prefs.dayOverrides).includes(x); })[0]; if (!d) return p; return { ...p, prefs: { ...p.prefs, dayOverrides: { ...p.prefs.dayOverrides, [d]: { latestEnd: "17:00", note: "" } } } }; }); } }} onClick={function () { setEditP(function (p) { var d = DAYS.filter(function (x) { return !Object.keys(p.prefs.dayOverrides).includes(x); })[0]; if (!d) return p; return { ...p, prefs: { ...p.prefs, dayOverrides: { ...p.prefs.dayOverrides, [d]: { latestEnd: "17:00", note: "" } } } }; }); }} style={{ fontSize: 11, color: Co.ac, cursor: "pointer", fontWeight: 600 }}>+ Add day</span></div>; })()}
</div>
</Collapsible>
</div>
)}
...
<div style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 11px", background: Co.acS, borderRadius: 6 }}><span style={{ color: Co.ac, fontWeight: 700, fontSize: 14 }}>{store.is24hr ? "E" : "O"}</span><span style={{ fontSize: 11, color: Co.txMu }}>/</span><span style={{ color: Co.pu, fontWeight: 700, fontSize: 14 }}>{store.is24hr ? "L" : "C"}</span><span style={{ fontSize: 11, color: Co.txMu }}>Split</span></div>
<div style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 11px", background: Co.amS, borderRadius: 6 }}><span style={{ color: Co.am, fontWeight: 700, fontSize: 14 }}>W</span><span style={{ fontSize: 11, color: Co.txMu }}>3+ on day</span></div>
</div>
...
return <div key={p.id + d} role="button" tabIndex={0} aria-label={(p.role === "pm" ? "PM" : firstName(p)) + " " + d + (val ? " " + val : " unassigned")} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); cycleCurAssign(wi, p.id, d); } }} onClick={function () { cycleCurAssign(wi, p.id, d); }} style={{ textAlign: "center", padding: "10px 0", minHeight: 44, fontSize: 16, fontWeight: 700, color: clr, background: bg, borderRadius: 6, cursor: "pointer", border: val ? "1.5px solid " + clr + "40" : "1.5px dashed " + Co.bdr, userSelect: "none", display: "flex", alignItems: "center", justifyContent: "center" }}>{label}</div>;
}));
})}
</div>
{(function () {
...
<div role="button" tabIndex={0} aria-expanded={isOpen} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setCurOverrides(function (o) { var no = { ...o }; no["*show*" + wi] = !o["*show*" + wi]; return no; }); } }} onClick={function () { setCurOverrides(function (o) { var no = { ...o }; no["*show*" + wi] = !o["*show*" + wi]; return no; }); }} style={{ fontSize: 11, color: Co.ac, cursor: "pointer", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}><span style={{ transform: isOpen ? "rotate(90deg)" : "none", transition: "transform 0.15s", fontSize: 12 }}>{"\u203A"}</span> Adjust shift times</div>
...
{assignedP.length < 3 ? <div role="checkbox" tabIndex={0} aria-checked={isOverlap} aria-label={"Overlap on " + d} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setCurOverrides(function (o) { var no = { ...o }; if (no[ovlKey]) delete no[ovlKey]; else no[ovlKey] = true; return no; }); } }} onClick={function () { setCurOverrides(function (o) { var no = { ...o }; if (no[ovlKey]) delete no[ovlKey]; else no[ovlKey] = true; return no; }); }} style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer", padding: "4px 10px", borderRadius: 6, background: isOverlap ? Co.acS : Co.card, border: "1px solid " + (isOverlap ? Co.ac + "40" : Co.bdr) }}><div style={{ width: 14, height: 14, borderRadius: 3, border: "1.5px solid " + (isOverlap ? Co.ac : Co.txD), display: "flex", alignItems: "center", justifyContent: "center", background: isOverlap ? Co.ac : "transparent" }}>{isOverlap ? <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>{"\u2713"}</span> : null}</div><span style={{ fontSize: 11, fontWeight: 600, color: isOverlap ? Co.ac : Co.txMu }}>Overlap</span></div> : null}
</div>
...
{R > 1 ? <div style={{ display: "flex", gap: 2 }}>{(function () { var tabs = []; for (var wi = 0; wi < R; wi++) tabs.push(wi); return tabs; })().map(function (wi) { return <div key={wi} role="tab" tabIndex={0} aria-selected={showWk === wi} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setShowWk(wi); } }} onClick={function () { setShowWk(wi); }} style={{ padding: "3px 11px", borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: "pointer", background: showWk === wi ? Co.ac + "18" : "transparent", color: showWk === wi ? Co.ac : Co.txD, border: showWk === wi ? "1px solid " + Co.ac + "30" : "1px solid transparent" }}>Wk {wi + 1}</div>; })}</div> : null}
<div style={{ flex: 1 }} />
...
{mode === "improve" && curScore ? <div role="button" tabIndex={0} aria-expanded={resultPanel === "compare"} aria-label="Compare vs current schedule" onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); togPanel(resultPanel === "compare" ? null : "compare"); } }} onClick={function () { togPanel(resultPanel === "compare" ? null : "compare"); }} style={{ background: Co.card, borderRadius: 11, boxShadow: shadow.sm, padding: "12px 16px", marginBottom: 8, border: "1px solid " + Co.bdrL, cursor: "pointer" }}>
...
<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, paddingLeft: 4 }}><div style={{ fontSize: 13, fontWeight: 700, color: Co.tx }}>Preference Impact</div><span role="button" tabIndex={0} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); togPanel("conflict"); } }} onClick={function () { togPanel("conflict"); }} style={{ fontSize: 11, color: Co.ac, cursor: "pointer", fontWeight: 600, marginLeft: "auto" }}>Full checklist {"\u203A"}</span></div>
...
{allHonored ? <div role="button" tabIndex={0} aria-expanded={resultPanel === "allHonored"} aria-label="What if we honored everything" onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); togPanel(resultPanel === "allHonored" ? null : "allHonored"); } }} onClick={function () { togPanel(resultPanel === "allHonored" ? null : "allHonored"); }} style={{ background: Co.card, borderRadius: 11, boxShadow: shadow.sm, padding: "12px 16px", marginBottom: 8, border: "1px solid " + Co.bdrL, cursor: "pointer" }}>
<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
...
return <div key={tab.k} role="button" tabIndex={0} aria-expanded={isActive} aria-label={tab.l} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); if (tab.k === "scores" && (resultPanel === "scores" || resultPanel === "scoresCur")) { setResultPanel(null); } else { togPanel(tab.k); } } }} onClick={function () { if (tab.k === "scores" && (resultPanel === "scores" || resultPanel === "scoresCur")) { setResultPanel(null); } else { togPanel(tab.k); } }} style={{ padding: "11px 12px", borderRadius: 8, background: isActive ? tab.c + "10" : Co.card, border: "1px solid " + (isActive ? tab.c + "40" : Co.bdrL), cursor: "pointer", boxShadow: shadow.sm, display: "flex", alignItems: "center", gap: 8 }}><div style={{ fontSize: 14, flexShrink: 0 }}>{tab.ico}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 600, color: isActive ? tab.c : Co.tx }}>{tab.l}</div><div style={{ fontSize: 11, color: Co.txMu }}>{tab.sub}</div></div><span style={{ fontSize: 12, color: Co.txD, transform: isActive ? "rotate(90deg)" : "none", transition: "transform 0.15s" }}>{"\u203A"}</span></div>;
})}
</div>
...
{curScore ? <div style={{ display: "flex", gap: 0, marginBottom: 12, background: Co.bg, borderRadius: 8, padding: 3 }}><div role="tab" tabIndex={0} aria-selected={resultPanel === "scores"} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setResultPanel("scores"); } }} onClick={function () { setResultPanel("scores"); }} style={{ flex: 1, textAlign: "center", padding: "6px 0", fontSize: 11, fontWeight: 700, borderRadius: 6, cursor: "pointer", background: resultPanel === "scores" ? Co.card : "transparent", color: resultPanel === "scores" ? Co.ac : Co.txMu, boxShadow: resultPanel === "scores" ? shadow.sm : "none" }}>New ({sc.grade})</div><div role="tab" tabIndex={0} aria-selected={resultPanel === "scoresCur"} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setResultPanel("scoresCur"); } }} onClick={function () { setResultPanel("scoresCur"); }} style={{ flex: 1, textAlign: "center", padding: "6px 0", fontSize: 11, fontWeight: 700, borderRadius: 6, cursor: "pointer", background: resultPanel === "scoresCur" ? Co.card : "transparent", color: resultPanel === "scoresCur" ? gc(curScore.grade) : Co.txMu, boxShadow: resultPanel === "scoresCur" ? shadow.sm : "none" }}>Current ({curScore.grade})</div></div> : null}
...
<div style={{ display: "flex", justifyContent: "center", padding: "12px 0 4px" }}><Btn onClick={function () { navigator.clipboard.writeText(reportText).then(function () { setCopied(true); setTimeout(function () { setCopied(false); }, 2000); }); }} style={{ padding: "8px 20px", fontSize: 12 }}>{copied ? "\u2713 Copied" : "\uD83D\uDCCB Copy Report"}</Btn></div>
