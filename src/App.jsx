...
<div role="button" tabIndex={0} aria-label="Improve current schedule" onClick={function () { setMode("improve"); }} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setMode("improve"); } }} style={{ background: "linear-gradient(135deg, #3D9A6D 0%, #2D7A54 60%, #236645 100%)", borderRadius: 16, boxShadow: "0 8px 32px rgba(61,154,109,0.25)", padding: "28px 22px 30px", cursor: "pointer", marginBottom: 14, color: "#fff", position: "relative", overflow: "hidden" }}>
...
<div role="button" tabIndex={0} aria-label="Build a new template" onClick={function () { setMode("scratch"); }} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setMode("scratch"); } }} style={{ padding: "22px 22px", cursor: "pointer", borderRadius: 16, background: Co.card, boxShadow: shadow.md, border: "1px solid " + Co.bdr, position: "relative", overflow: "hidden" }}>
...
<div className="toggle-group-option" onClick={function () { setStore(function (s) { return { ...s, _hoursChanged: false, allocatedHoursPerWeek: s._originalBudget || s.allocatedHoursPerWeek }; }); }} style={{ background: !store._hoursChanged ? Co.ac : "transparent", color: !store._hoursChanged ? "#fff" : Co.txMu }}>Same for new template</div>
<div className="toggle-group-option" onClick={function () { setStore(function (s) { return { ...s, _hoursChanged: true, _originalBudget: s._originalBudget || s.allocatedHoursPerWeek }; }); }} style={{ background: store._hoursChanged ? Co.am : "transparent", color: store._hoursChanged ? "#fff" : Co.txMu, borderLeft: "1px solid " + Co.bdr }}>Changed</div>
</div>
...
<div className="toggle-group" style={{ border: "1px solid " + Co.bdr }}>
<div className="toggle-group-option" onClick={function () { setStore(function (s) { return { ...s, _rotChanged: false, rotationWeeks: s._originalRot || s.rotationWeeks }; }); }} style={{ background: !store._rotChanged ? Co.ac : "transparent", color: !store._rotChanged ? "#fff" : Co.txMu }}>Same for new template</div>
<div className="toggle-group-option" onClick={function () { setStore(function (s) { return { ...s, _rotChanged: true, _originalRot: s._originalRot || s.rotationWeeks }; }); }} style={{ background: store._rotChanged ? Co.am : "transparent", color: store._rotChanged ? "#fff" : Co.txMu, borderLeft: "1px solid " + Co.bdr }}>Changed</div>
</div>
...
<span role="button" tabIndex={0} aria-label={"Remove " + p.name} onClick={function () { setRemoving(removing === p.id ? null : p.id); }} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setRemoving(removing === p.id ? null : p.id); } }} style={{ fontSize: 12, color: Co.txD, cursor: "pointer", padding: "8px 10px", margin: "-8px -10px" }}>{"\u2715"}</span>
</div>
...
<div className="toggle-group" style={{ border: "1px solid " + Co.bdr, marginBottom: 10 }}>
<div className="toggle-group-option" onClick={function () { setEditP(function (p) { var b = +p.targetHours || 40; return { ...p, _showRange: false, minHours: b - 8, maxHours: b + 8 }; }); }} style={{ background: !hasRange ? Co.ac : "transparent", color: !hasRange ? "#fff" : Co.txMu }}>Exact hours</div>
<div className="toggle-group-option" onClick={function () { setEditP(function (p) { return { ...p, _showRange: true }; }); }} style={{ background: hasRange ? Co.am : "transparent", color: hasRange ? "#fff" : Co.txMu, borderLeft: "1px solid " + Co.bdr }}>Acceptable range</div>
</div>
...
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><span style={{ fontSize: 12, flex: 1 }}>Max days/week</span><Sel value={editP.prefs.maxDaysPerWeek || ""} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, maxDaysPerWeek: v === "" ? null : +v } }; }); }} style={{ flex: 1 }}><option value="">No cap</option>{[3,4,5,6].map(function (n) { return <option key={n} value={n}>{n}</option>; })}</Sel></div>
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><span style={{ fontSize: 12, flex: 1 }}>Max consecutive days</span><Sel value={editP.prefs.maxConsecutiveWorkDays} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, maxConsecutiveWorkDays: +v } }; }); }} style={{ width: 60 }}>{[3,4,5,6,7].map(function (n) { return <option key={n} value={n}>{n}</option>; })}</Sel></div>
<div className="pref-row" style={{ borderBottom: "1px solid " + Co.bdrL }}><span style={{ fontSize: 12, flex: 1 }}>Min consecutive days off</span><Sel value={editP.prefs.consecutiveDaysOff} onChange={function (v) { setEditP(function (p) { return { ...p, prefs: { ...p.prefs, consecutiveDaysOff: +v } }; }); }} style={{ width: 60 }}>{[1,2,3].map(function (n) { return <option key={n} value={n}>{n}</option>; })}</Sel></div>
{/* TIME CONSTRAINTS */}
...
{mode === "improve" && curScore ? <div role="button" tabIndex={0} aria-label="Compare with current schedule" onClick={function () { togPanel(resultPanel === "compare" ? null : "compare"); }} onKeyDown={function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); togPanel(resultPanel === "compare" ? null : "compare"); } }} style={{ background: Co.card, borderRadius: 11, boxShadow: shadow.sm, padding: "12px 16px", marginBottom: 8, border: "1px solid " + Co.bdrL, cursor: "pointer" }}>
